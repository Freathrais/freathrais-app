  ...
  return (
    <div className="mt-2 space-y-1.5">
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-0.5">
        {checks.map((c) => (
          <span key={c.label} className="flex items-center gap-1 text-xs" style={{ color: c.ok ? "var(--success)" : "var(--text-subtle)" }}>
...
export default function RegisterPage() {
  ...
  const handleGithub = async () => {
    ...
    try {
      ...
      setError("GitHub ile kayıt başarısız. Lütfen tekrar deneyin.");
    } finally {
      setLoading(null);
    }
  };
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            Ücretsiz kayıt ol, topluluğa katıl.
          </p>
        </div>
        ...
        <div className="space-y-3 mb-6">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl text-sm font-semibold transition-all duration-200 hover:opacity-90 hover:scale-[1.02] disabled:opacity-50"
            style={{ background: "white", color: "#1f1f1f", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}
          >
            {loading === "google" ? <div className="w-4 h-4 rounded-full border-2 border-gray-400 border-t-transparent animate-spin" /> : (
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl text-sm font-semibold transition-all duration-200 hover:opacity-90 hover:scale-[1.02] disabled:opacity-50"
            style={{ background: "#24292e", color: "white", boxShadow: "0 2px 8px rgba(0,0,0,0.3)" }}
          >
            {loading === "github" ? <div className="w-4 h-4 rounded-full border-2 border-gray-400 border-t-transparent animate-spin" /> : <GithubIcon />}
            GitHub ile Kayıt Ol
          </button>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-xs px-2" style={{ color: "var(--text-subtle)" }}>veya e-posta ile</span>
          <div className="flex-1 h-px" style={{ background: "var(--border)" }} />
        </div>
        ...
        <form onSubmit={handleEmail} className="space-y-4">
          ...
          <button type="submit" disabled={loading !== null} id="btn-email-register"
            className="w-full py-3.5 rounded-2xl text-sm font-semibold transition-all duration-200 hover:opacity-90 hover:scale-[1.02] disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
            style={{ background: "var(--primary)", color: "var(--primary-fg)", boxShadow: "0 0 20px var(--glow)" }}>
            {loading === "email" ? <div className="w-4 h-4 rounded-full border-2 border-current border-t-transparent animate-spin" /> : null}
            Hesap Oluştur
          </button>
        </form>
        ...
        <p className="text-center text-sm mt-6" style={{ color: "var(--text-muted)" }}>
          ...
          <Link href="/giris" className="font-semibold hover:opacity-70" style={{ color: "var(--accent)" }}>Giriş Yap</Link>
        </p>
      </div>
    </motion.div>
  );
